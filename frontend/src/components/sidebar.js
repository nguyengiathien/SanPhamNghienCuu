        ...
        <aside className="z-3 bg-gradient-to-b from-indigo-600 to-violet-800 w-[50px] h-screen shadow-[0_25px_20px_var(--tw-shadow-color,_rgba(0,_0,_0,.5))] fixed">
            ...
            <nav className="p-4 mt-[2rem] relative">
                <ul className="flex flex-col gap-4 absolute left-2 top-0">
                    ...
                    <li className={`group/icon mb-2  left-5 flex flex-row items-center ${userData !== null && userData !== undefined && userData.role === "admin" ? "show" : "hidden"}`}>
                        ...
                        <a href="#classes" className="text-indigo-400 hover:text-gray-800 absolute left-3 z-0 py-1 pl-3 pr-2 rounded-r-full font-semibold bg-white shadow-[0_2px_4px_rgba(1,1,1,0.2)] opacity-0 transition delay-150 duration-300 ease-in-out group-hover/icon:translate-x-5 group-hover/icon:opacity-100 hover:block text-sm text-nowrap text-nowrap">Quản lý người dùng</a>
                    </li>
                    <li className={`group/icon mb-2  left-5 flex flex-row items-center ${userData !== null && userData !== undefined && userData.role === "admin" ? "show" : "hidden"}`}>
                        <FontAwesomeIcon icon={faLinesLeaning} className="mr-2 p-[8px] text-white z-1 bg-indigo-400 rounded-[50%] !w-[22px] !h-[22px] hover:shadow-[0_2px_4px_rgba(1,1,1,0.2)] transition-shadow duration-300" />
                        <a href="#lessons" className="text-indigo-400 hover:text-gray-800 absolute left-3 z-0 py-1 pl-3 pr-2 rounded-r-full font-semibold bg-white shadow-[0_2px_4px_rgba(1,1,1,0.2)] opacity-0 transition delay-150 duration-300 ease-in-out group-hover/icon:translate-x-5 group-hover/icon:opacity-100 hover:block text-sm text-nowrap text-nowrap">Quản lý khóa học</a>
                    </li>
                    <li className={`group/icon mb-2  left-5 flex flex-row items-center ${userData !== null && userData !== undefined && userData.role === "admin" ? "show" : "hidden"}`}>
                        <FontAwesomeIcon icon={faChalkboard} className="mr-2 p-[8px] text-white z-1 bg-indigo-400 rounded-[50%] !w-[22px] !h-[22px] hover:shadow-[0_2px_4px_rgba(1,1,1,0.2)] transition-shadow duration-300" />
                        <a href="#lessons" className="text-indigo-400 hover:text-gray-800 absolute left-3 z-0 py-1 pl-3 pr-2 rounded-r-full font-semibold bg-white shadow-[0_2px_4px_rgba(1,1,1,0.2)] opacity-0 transition delay-150 duration-300 ease-in-out group-hover/icon:translate-x-5 group-hover/icon:opacity-100 hover:block text-sm text-nowrap text-nowrap">Quản lý lớp học</a>
                    </li>
                </ul>
            </nav>
